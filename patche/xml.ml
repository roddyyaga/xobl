...
module Attr = struct
  ...
  let required name ls =
    ...
    | Some v, rest ->
        Ok (v, rest)
    | None, _ ->
        Error (`Attribute_not_found name)

  let eoi = function
    | [] ->
        Ok ((), [])
    | _ ->
        Error `Trailing_elements_detected
  ...
  let int name =
    ...
    int_of_string_opt v
    |> Option.to_result ~none:(`Conversion_failed (name, "int", v))
    |> from_result
...
let el_start_any =
  apply
  @@ function `El_start _ -> Ok () | s -> Error (`Expected_el_start ("", s))
...
let el_start_a name attr =
  ...
  | `El_start ((_, n), attrs) when n = name ->
      (Attr.run attr) attrs
  | s ->
      Error (`Expected_el_start (name, s))
